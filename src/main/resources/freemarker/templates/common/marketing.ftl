                ...
                <div style="width: ${galleryWidth}px" class="gallery clearfix">
                    ...
                    <#list items as item>
                        <div style="cursor: pointer; width: ${itemWidth}px" class="gallery__item <#if x == 0>gallery__item--active</#if> text-center" >
                            ...
                            <h2>${item.getName()}</h2>
                            <div class="item-price-small"><#if isClassic == true><span>${translator.getString("from")} </span></#if>${(item.getPrice() * item.getMinAmount())?string?replace(",",".")}${translator.getString("currency")}</div>
                            </div>
